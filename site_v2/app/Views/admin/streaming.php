	...
	<div class="container-fluid">
      ...
      <div class="col-lg-8">
        <?php if(isset($_POST['submit'])){
          ...
          if($request->GetPost('season_id') != $streams['season_id']){
            echo "<div class='alert alert-success'>Vous avez modifié l'id de la saison !</div>";
            $builder->set('season_id', $request->GetPost('season_id'));
            $builder->where('id', $streams['id']);
            $builder->update();
          }

          if($request->GetPost('episode_id') != $streams['episode_id']){
            echo "<div class='alert alert-success'>Vous avez modifié l'id de l'épisode !</div>";
            $builder->set('episode_id', $request->GetPost('episode_id'));
            $builder->where('id', $streams['id']);
            $builder->update();
          }
        }
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row mb-3">
              <div class="col-sm-3">
                <h6 class="mb-0">Choix de la saison</h6>
              </div>
              <div class="col-sm-9 text-secondary">
                <select class="form-control" name="season_id">

                    <option value="0" <?php if($row_server->id === $streams['season_id']){ ?>selected<?php } ?>>Aucun</option>
                  <?php $builder_season = $db->table('categories');
                  $builder_season->where('id_season != ', 0);
                  $builder_season->orderBy('id', 'DESC');
                  $query_season = $builder_season->get();
                  foreach ($query_season->getResult() as $row_server){ ?>
                    <option value="<?= $row_server->id; ?>" <?php if($row_server->id === $streams['season_id']){ ?>selected<?php } ?>><?= $row_server->name; ?></option>
                  <?php } ?>
                </select>
              </div>
            </div>
            <div class="row mb-3">
              <div class="col-sm-3">
                <h6 class="mb-0">Numéro de l'épisode</h6>
              </div>
              <div class="col-sm-9 text-secondary">
                <input type="number" name="episode_id" class="form-control" value="<?= $streams['episode_id']; ?>" />
              </div>
            </div>
